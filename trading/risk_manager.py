...
class RiskManager:
    ...
    def check_risk(self, trade_amount, current_loss, params):
        """检查交易风险"""
        if trade_amount > self.total_funds * params["max_position"] / 100:
            logger.warning("交易金额超过最大持仓限制")
            return False
        if self.daily_loss + current_loss >= self.total_funds * params["circuit_breaker_loss"] / 100:
